...
export class QueueProcessor {
  ...
  async handleNewPost(job: Job) {
    ...
    try {
      const completion = await this.openApiService.generateContent(job.data.title)
      ...
      const newPostId = await this.strapiService.createArticle(job.data.title, completion)
    ...
    this.logger.verbose(`Job ${job.id} completed in ${(Date.now() - start) / 1000} s`)
  }
}
